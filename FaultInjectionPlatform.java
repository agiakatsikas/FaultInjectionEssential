...
/*		FrameExtractor extractor = new FrameExtractor();
		...
		String ebdFilePath = fullBitstream.replaceAll(".bit", ".ebd");
		String ebcFilePath = fullBitstream.replaceAll(".bit", ".ebc");
		
		//String xdcFilePath = "E:\\FCCM18\\Test\\aes_0\\fie\\fie.srcs\\constrs_1\\imports\\constraints\\fie.xdc";
		...
		EssentialBits7Series ebitAnalysis = new EssentialBits7Series(fullBitstream, ebdFilePath, ebcFilePath);
		ebitAnalysis.printResults();
		
		//Remove MB, IOB, CMT, CLOCK, ICAP columns from the FADs
		HashMap<Integer, ArrayList<Integer>> sensitiveFramesFull = ebitAnalysis.getSenitiveFramesFull();
		HashMap<Integer, ArrayList<Integer>> sensitiveFrames = new HashMap<>();
		
		Set<Integer> FADs = sensitiveFramesFull.keySet();
		for(Integer frameAddress : FADs) {
			if( (Artix200t.getColumnType(frameAddress) == Artix200t.ColumnType.CLB 
					|| Artix200t.getColumnType(frameAddress) == Artix200t.ColumnType.DSP	
					|| Artix200t.getColumnType(frameAddress) == Artix200t.ColumnType.BRAM 
					|| Artix200t.getColumnType(frameAddress) == Artix200t.ColumnType.CLOCK_INTER
					
				) == true) { 
				int column =  Artix200t.getColumnNumber(frameAddress);
				int row = Artix200t.getRowNumber(frameAddress);
				int top = ( frameAddress >> 22 ) & 0x00000001;
				if( (top == 1) && (row == 0) && (column < 35)) {
					continue;
				} else {
					sensitiveFrames.put(frameAddress, sensitiveFramesFull.get(frameAddress));
				}
		...
		numberInject = ebitAnalysis.countBitsHashMap(sensitiveFrames);
		System.out.println(numberInject);
		RobustFaultInjector injector = new RobustFaultInjector(sensitiveFrames, percent, percentBoolean, numberInject, serialA,  logger,  expected, fmax, latency, jtag, guiEnable, vivadoPath);
