...
public class FaultInjector {
	...
	private HashMap<Integer, ArrayList<Integer>> sensitiveFrames;
	...
	{
		this.sensitiveFrames = sensitiveFrames;
	...
	int reset_count = 0;
	Set<Integer> FADs = sensitiveFrames.keySet();
	for(Integer frameAddress : FADs) {
		/*if( (Artix200t.getColumnType(frameAddress) == Artix200t.ColumnType.CLB 
				...
				|| Artix200t.getColumnType(frameAddress) == Artix200t.ColumnType.BRAM) == false) { 
			continue;
		}
		...
		int row = Artix200t.getRowNumber(frameAddress);
		int top = ( frameAddress >> 22 ) & 0x00000001;
		System.out.println("T = " + top +" C = " + column + " r = " + row );*/
		ArrayList<Integer> sensitiveBits = sensitiveFrames.get(frameAddress);
		for(Integer bitPosition : sensitiveBits) {
			
			//System.out.println(Artix200t.getColumnType(frameAddress));
